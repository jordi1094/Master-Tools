...
describe('getCharacters', () => {
    ...
    it('succesfully finds all Charactters in the array', () => {
        ...
        .then(user => {
            ...
            return Promise.all([
                ...
                const charactersIds = [testCharacter1._id.toString(), testCharacter2._id.toString()]

                return getCharacters(userId, charactersIds)
                    .then(characters => {
                        expect(characters.length).to.equal(2)
                        characters.forEach((character, index) => {
                            const expectedCharacter = index === 0 ? testCharacter1 : testCharacter2
                            expect(character).to.exist
                            expect(character.id).to.equal(expectedCharacter._id.toString())
                            expect(character.name).to.equal(expectedCharacter.name)
                            expect(character.classCharacter).to.equal(expectedCharacter.classCharacter)
                            expect(character.speed).to.equal(expectedCharacter.speed)
                            expect(character.gold).to.equal(expectedCharacter.gold)
                        })
                    })
            })
        })
    ...
    it('fails on non-existing user', () => {
        ...
        .then(user => {
            ...
            return Promise.all([
                ...
                return getCharacters(new ObjectId().toString(), charactersIds)
    ...
    it('fails on non-existing one character in the array', () => {
        let errorThrown
        return bcrypt.hash('123123123', 8)
        .then(hash => User.create({name: 'Mac', surname: 'Book', email: 'mac@book.com', username: 'macbook', role: 'master', password: hash }))
        .then(user => {
            ...
            return Promise.all([
                ...
                const charactersIds = [new ObjectId().toString(), testCharacter2._id.toString()]

                return getCharacters(userId, charactersIds)
                .catch(error => errorThrown = error)
                .finally(() => {
                    expect(errorThrown).to.be.an.instanceOf(NotFoundError);
                    expect(errorThrown.message).to.equal('One o more characters not found');
                });
            })
        })
    })
